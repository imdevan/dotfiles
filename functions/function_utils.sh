...
function vim_function() {
  ...
  if [ -z "$1" ]; then
    with_vim "$functions_path/"
  else
    local function_name=$(to_snake_case "$@")

    if [[ "${function_name: -3}" == ".sh" ]]; then
      with_vim "$functions_path/$function_name"
    else 
      with_vim "$functions_path"/*"$function_name"*

      # other attempts. consider these if the above in sufficient
      # find . -iname "*partial_filename*" -print0 | xargs -0 with_vim
      # with_vim '$(find "$functions_path" -type f -name "*$function_name"" | head -n 1)'
    fi
  fi
}
